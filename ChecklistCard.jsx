...
const FUENTES = [
  { value: 'fijo', label: 'Gastos fijos (ya presupuestado)' },
  { value: 'deuda', label: 'Deuda (ya presupuestada)' },
  { value: 'personal', label: 'Gastos personales (Dinero para mi)' },
  { value: 'flujoLibre', label: 'Flujo libre' },
];
    ...
    <div className="fin-card rounded-2xl p-5 sm:p-6 border border-[#1A1A1A] bg-[#141414] flex flex-col w-full">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <span className="text-[19px] font-display text-[#9A9AA0] flex items-center gap-1.5">
          <ListChecks size={17} /> Checklist del mes
        </span>
        <span className="text-[12.5px] text-[#5A5A5A] font-mono">
          {ejecutados}/{conEstado.length} ejecutados · {pendientes} pendientes
        </span>
      </div>

      <div className="grid sm:grid-cols-3 gap-8 mt-5">
        {grupos.map((g) => (
          <div key={g.grupo}>
            <p className="text-[12.5px] uppercase tracking-wide text-[#5A5A5A] mb-3">{g.label}</p>
...
function ChecklistRow({ item, onEjecutar, onRevertir }) {
  ...
  return (
    ...
    >
      <span
        className={`w-5 h-5 rounded-[6px] border flex items-center justify-center flex-shrink-0 transition-colors ${
          ejecutado ? 'bg-[#4ADE80] border-[#4ADE80]' : 'border-[#333] group-hover:border-[#666]'
        }`}
      >
        {ejecutado && <Check size={12} className="text-black" />}
      </span>
      <span className="flex-1 min-w-0">
        <p className={`text-[14.5px] leading-tight truncate ${ejecutado ? 'text-[#5A5A5A] line-through' : 'text-[#D4D4D4]'}`}>
          {item.concepto}
        </p>
        <p className="text-[12px] text-[#5A5A5A] font-mono mt-0.5">
          {ejecutado ? formatMoney(item.override.montoReal) : formatMoney(item.montoPlan)}
          {ejecutado && item.override.fuente && (
            <span className="text-[#454545]"> · {FUENTES.find((f) => f.value === item.override.fuente)?.label.split(' (')[0]}</span>
          )}
        </p>
      </span>
    </button>
...
function EjecutarModal({ item, onClose }) {
  ...
  const [fuente, setFuente] = useState(item.etiquetaMovimiento || 'fijo');

  const requiereFuente = item.grupo !== 'AHORRO_METAS';

  const submit = (e) => {
    e.preventDefault();
    marcarChecklistEjecutado(item, { montoReal: Number(montoReal), fecha, fuente: requiereFuente ? fuente : null });
    onClose();
  };
  ...
  return (
    <Modal title={`Ejecutar — ${item.concepto}`} onClose={onClose} width="max-w-[400px]">
      <form onSubmit={submit} className="flex flex-col gap-3.5">
        ...
        {requiereFuente && (
          <div>
            <FieldLabel>¿De donde salio el dinero?</FieldLabel>
            ...
            <p className="text-[11px] text-[#5A5A5A] mt-1.5">
              Esto determina si afecta tu Saldo disponible o si ya estaba contemplado en el plan.
            </p>
          </div>
        )}
