...
export default function DistribucionIngresoCard() {
  ...
  return (
    <div className="fin-card h-full flex flex-col rounded-2xl p-5 border border-[#1A1A1A] bg-[#141414]">
      ...
      <div className="flex flex-col items-center gap-4 mt-3 flex-1 justify-center">
        <div className="relative w-[140px] h-[140px] flex-shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip
                ...
                formatter={(value, name, props) => [formatMoney(value) + ` (${formatPct(props.payload.pct, 0)})`, name]}
              />
            </PieChart>
          </ResponsiveContainer>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none px-2 text-center">
            {active ? (
              <>
                <span className="text-[10px] uppercase tracking-wide text-[#75757A] truncate max-w-[90px]">{active.label}</span>
                <span className="font-display text-[17px] font-medium text-[#F2F2F2] leading-tight mt-0.5">{formatMoney(active.valor)}</span>
                <span className="text-[12px] font-mono mt-0.5" style={{ color: active.color }}>
                  {formatPct(active.pct, 0)}
                </span>
              </>
            ) : (
              <>
                <span className="text-[10px] uppercase tracking-wide text-[#75757A]">Total</span>
                <span className="font-display text-[17px] font-medium text-[#F2F2F2] leading-tight mt-0.5">{formatMoney(total)}</span>
              </>
            )}
          </div>
        </div>
          ...
          {data.map((d, i) => (
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                />
                <span className={`text-[13px] truncate ${activeIndex === i ? 'text-[#F2F2F2]' : 'text-[#9A9AA0]'}`}>{d.label}</span>
              </div>
              <span className="text-[12px] font-mono text-[#5A5A5A] flex-shrink-0">{formatPct(d.pct, 0)}</span>
            </button>
          ))}
        </div>
      </div>
