...
// eliminar) y esto deja de tener efecto. El plan real actual (segun este
// mensaje) SI contempla un abono extraordinario de $2.300.000 en diciembre
// 2026 -- se registra como un pago mas, no como una regla especial: el
// cronograma dinamico ya sabe usar el pago real de un mes en vez de la cuota
// base cuando existe uno (ver simularCronogramaDeuda).
function deudaUniversidadSemilla() {
  return {
    ...
    notas: 'Cargada segun los datos entregados el 13-ago-2026. Incluye abono extraordinario de diciembre 2026.',
    estado: 'ACTIVA',
    esUniversidad: true,
    createdAt: Date.now(),
    updatedAt: Date.now(),
...
// Abono extraordinario ya confirmado por el usuario (mensaje del 24-ago-2026):
// $2.300.000 en diciembre 2026. Se siembra como un PAGO real (no como un
// caso especial en el codigo) para que el cronograma, saldo y presupuesto de
// diciembre lo reflejen automaticamente -- y para que sea editable/eliminable
// desde la UI exactamente igual que cualquier otro pago.
function pagoExtraordinarioDiciembreSemilla(deudaId) {
  return {
    id: 'pago-extraordinario-dic-2026',
    deudaId,
    fecha: '2026-12-15',
    monto: 2300000,
    tipo: 'extraordinario',
    nota: 'Abono extraordinario planeado (confirmado 24-ago-2026)',
    createdAt: Date.now(),
    updatedAt: Date.now(),
  };
}
...
function initialState() {
  ...
  const deudas = persisted?.deudas || [deudaUniversidadSemilla()];
  const universidad = deudas.find((d) => d.esUniversidad);
  const pagosDeuda = persisted?.pagosDeuda || (universidad ? [pagoExtraordinarioDiciembreSemilla(universidad.id)] : []);
  return {
