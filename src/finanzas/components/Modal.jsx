...
export function ConfirmModal({ title, message, confirmLabel = 'Eliminar', danger = true, onConfirm, onClose }) {
  return (
    <Modal title={title} onClose={onClose} width="max-w-[360px]">
      <p className="text-[13px] text-[#9A9AA0] leading-relaxed">{message}</p>
      <div className="flex gap-2.5 mt-5">
        ...
        <button
          ...
          className={`flex-1 py-2.5 rounded-full text-[13px] font-normal transition-colors ${
            danger ? 'bg-[#F2685C] text-white hover:bg-[#E0544A]' : 'bg-[#3B82F6] text-white'
          }`}
        >
          {confirmLabel}
        </button>
