...
export default function ChecklistCard() {
  const { planOficial, mesActivo, checklistOverrides, marcarChecklistPendiente } = useFinance();
  const [ejecutarItem, setEjecutarItem] = useState(null);

  const plan = getPlanMensual(planOficial, mesActivo);
    ...
    <div className="rounded-2xl p-5 border border-[#1A1A1A] bg-[#141414] lg:col-span-2 flex flex-col">
      <div className="flex items-center justify-between">
        <span className="text-[11.5px] text-[#75757A] flex items-center gap-1.5">
          <ListChecks size={12} /> Checklist del mes
        </span>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mt-4">
        {grupos.map((g) => (
          <div key={g.grupo}>
            <p className="text-[10.5px] uppercase tracking-wide text-[#5A5A5A] mb-2">{g.label}</p>
...
function ChecklistRow({ item, onEjecutar, onRevertir }) {
  ...
  return (
    ...
    >
      ...
      >
        {ejecutado && <Check size={10} className="text-black" />}
      </span>
      <span className="flex-1 min-w-0">
        <p className={`text-[12px] leading-tight truncate ${ejecutado ? 'text-[#5A5A5A] line-through' : 'text-[#D4D4D4]'}`}>
          {item.concepto}
        </p>
