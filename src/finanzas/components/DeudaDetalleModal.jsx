...
export default function DeudaDetalleModal({ deudaIdInicial, onClose }) {
  ...
  // Editar una celda de "Cuota" en el cronograma equivale a registrar/ajustar
  // el pago real de ese mes -- asi el saldo, las cuotas futuras y la fecha
  // final se recalculan solos (todo se deriva del mismo cronograma dinamico).
  function guardarCuotaMes(mes, nuevoValor) {
    const pagosDelMes = pagos.filter((p) => p.fecha.slice(0, 7) === mes);
    if (pagosDelMes.length === 1) {
      editPagoDeuda(pagosDelMes[0].id, { monto: nuevoValor });
    } else if (pagosDelMes.length === 0) {
      const cuotaBase = Number(deuda.cuotaRegular || 0) + Number(deuda.seguro || 0);
      addPagoDeuda({
        deudaId: deuda.id,
        fecha: mes + '-15',
        monto: nuevoValor,
        tipo: Math.abs(nuevoValor - cuotaBase) < 1 ? 'regular' : 'extraordinario',
        nota: 'Editado manualmente desde el cronograma',
      });
    } else {
      // Varios pagos ya registrados ese mes: en vez de adivinar cual tocar,
      // se agrega un ajuste que deja la suma del mes en el valor pedido.
      const totalActual = pagosDelMes.reduce((s, p) => s + p.monto, 0);
      const diferencia = nuevoValor - totalActual;
      if (Math.abs(diferencia) > 0.5) {
        addPagoDeuda({ deudaId: deuda.id, fecha: mes + '-20', monto: diferencia, tipo: 'ajuste', nota: 'Ajuste desde edicion del cronograma (varios pagos ese mes)' });
      }
    }
  }
  ...
  return (
    <Modal title="Deudas — detalle" onClose={onClose} width="max-w-[840px]">
      ...
      <div className="mb-6">
        ...
        {!simulando && (
          <>
            <p className="text-[10.5px] text-[#5A5A5A] mb-2">Haz clic en cualquier cuota para editarla manualmente — el saldo y las cuotas futuras se recalculan solos.</p>
            <div className="rounded-xl border border-[#1E1E1E] overflow-hidden max-h-[260px] overflow-y-auto">
              <table className="w-full text-[12px]">
                ...
                <tbody className="divide-y divide-[#1E1E1E]">
                  {cronograma.map((f) => (
                    <tr key={f.mes} className={`text-[#D4D4D4] font-mono ${f.esHipotetico ? 'bg-[#F1BD3D]/[0.04]' : ''}`}>
                      <td className="px-3 py-2">{f.mesLabel}</td>
                      <td className="px-3 py-2 text-right">
                        <EditableCuotaCell mes={f.mes} valor={f.cuota} onGuardar={(nuevoValor) => guardarCuotaMes(f.mes, nuevoValor)} />
                      </td>
...
function EditableCuotaCell({ mes, valor, onGuardar }) {
  const [editando, setEditando] = useState(false);
  ...
  if (editando) {
    return (
      <input
        ...
        onBlur={() => {
          setEditando(false);
          const n = Number(valorTemp);
          if (!Number.isNaN(n) && n >= 0 && n !== valor) onGuardar(n);
        }}
        onKeyDown={(e) => {
          if (e.key === 'Enter') e.currentTarget.blur();
          if (e.key === 'Escape') { setValorTemp(valor); setEditando(false); }
        }}
        className="w-[110px] text-right bg-[#0F0F0F] border border-[#3B82F6] rounded px-1.5 py-0.5 text-[12px] text-[#E5E5E5] font-mono outline-none"
      />
    );
  }
  ...
  return (
    <button
      ...
      className="hover:text-[#3B82F6] hover:underline decoration-dotted underline-offset-2 transition-colors"
      title="Clic para editar esta cuota"
    >
      {formatMoney(valor)}
    </button>
  );
