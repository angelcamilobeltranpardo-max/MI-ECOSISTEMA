  ...
  return (
    <div className="fin-card rounded-2xl p-5 border border-[#1A1A1A] bg-[#141414] flex flex-col">
      <span className="text-[19px] font-display text-[#9A9AA0] flex items-center gap-1.5">
        <Gauge size={17} /> Daily Pacing
      </span>

      <p className="font-display text-[38px] font-semibold text-[#F2F2F2] leading-none mt-3 tracking-tight">
      ...
      <div className="mt-4">
        ...
        <div className="flex justify-between mt-2 text-[13.5px] text-[#75757A] font-mono">
          <span>Gastado: {formatMoney(pacing.gastado)}</span>
          <span>Restante: {formatMoney(pacing.restante)}</span>
        </div>
      </div>

      <p className="text-[12.5px] text-[#5A5A5A] mt-3 pt-3 border-t border-[#1E1E1E]">
        {esMesActual
